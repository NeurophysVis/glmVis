(function() {
  ...
  mainEffects.init = function(params) {
    ...
    avgFiringHeight = 100 - margin.top - margin.bottom;
    avgFiringChart = d3.select('#avgFiring-chart');
    avgFiringChart.selectAll('svg')
      .data([{
        width: width + margin.left + margin.right,
        height: avgFiringHeight + margin.top + margin.bottom
      }])
      .enter()
      .append('svg');

    avgFiringSVG = avgFiringChart.select('svg').attr({
      width: function(d) {
        return d.width + margin.left + margin.right;
      },
    ...
    avgFiringStyle = avgFiringSVG.selectAll('style').data([{}]).enter()
        .append('style')
        .attr('type', 'text/css');
  ...
  mainEffects.draw = function(params) {
    ...
    plotHistG = avgFiringSVG.selectAll('g.brainArea').data(neurons);
    plotHistG
          .enter()
          .append('g')
          .attr('transform', function(d, i) {
    ...
    function drawHist(brainArea) {
      ...
      foreRect
        ...
        .duration(1000)
          .attr('height', function(d) {
            return avgFiringHeight - yAvgFiringScale(d.y);
          })
          .attr('y', function(d) {
      ...
      histAxisG.enter()
        .append('g')
         .attr('class', 'axis')
         .attr('transform', 'translate(0,' + avgFiringHeight + ')');
      histAxisG
         .call(avgFiringAxis);
    ...
    function pathBrushed() {
      ...
      var activeNeuronsACC = d3.selectAll('#mainEffects-chart')
            .selectAll('#ACC')
            .selectAll('.foreground').selectAll('path')
            .filter(function() {
              return d3.select(this).style('display') == 'inline';
            }).data(),
          activeNeuronsDLPFC = d3.selectAll('#mainEffects-chart')
              .selectAll('#dlPFC')
              .selectAll('.foreground').selectAll('path')
              .filter(function() {
                return d3.select(this).style('display') == 'inline';
              }).data();
      var activeNeurons = activeNeuronsACC.concat(activeNeuronsDLPFC);
